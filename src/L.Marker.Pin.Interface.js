...
( function ( ){
	...
	v1.2.4:
	- the newPin ( ) method have an optionnal parameter 'options', so it's possible to createElement
	a pin with already filled values.
	- added code for distance
	- added readOnly getter and setter
	...
	*/
	...
	var _PinsHtmlOptions = { 
		...
		urlLength : 9999,
		distanceElement : 'div',
		distanceClass : 'Pin-Print-Distance',
	...
	L.Marker.Pin.getInterface = function ( ) {
		...
		return {
			...
			*/
			...
			- options : already completed options for the new pin - not mandatory.

			*/

			newPin : function ( Map, latlng, options ) {
				var EditDialog;
				if ( typeof module !== 'undefined' && module.exports ) {
				...
				options.exist = false;
				if ( options.pinCategoryId ) {
					options.pinCategory = _Categories.getCategory ( options.pinCategoryId );
				}
				
				new EditDialog ( Map, latlng, options ).show ( );
			},
			...
			*/
			...
			<div class ="Pin-Print-Main">
				<div class="Pin-Print-Pin">
					...
					<div class="Pin-Print-Distance">
					</div>
					<div class="Pin-Print-NextDistance">
					</div>
				</div>
				... next pin...
			</div>
