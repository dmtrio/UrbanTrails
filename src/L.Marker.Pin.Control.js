...
( function ( ) {
	...
	v1.2.4:
	- added a show / hide button in the control
	- added code to block the dblclick event when pins are read only
	- added code to close popup before click and dblclick event to avoid a crash with leaflet
	...
	var _onDblClick = function ( MouseEvent ) { 
		...
		if ( SelectedElement && SelectedElement.className && ( -1 !== SelectedElement.className.indexOf ('PinControl-Pin' ) ) ) {
			...
			var options = {
				...
				distance : ( Pin.options.distance ? Pin.options.distance : 0 ),
	...
	*/
	...
	--- _onClickShowHide( MouseEvent ) ---
	
	onclick event handler for the show/hide pins button
	
	This event show or hide the pins
	
	*/
	var _onClickShowHide = function ( MouseEvent ) {
		if ( _PinsVisible ) {
			MouseEvent.target.id = 'PinControl-ShowButton';
			MouseEvent.target.setAttribute ( 'title' , _Translator.getText ( 'L.Marker.Pin.Control.onAdd.ShowButton' ) );
			_Pins.hide ( );
			_PinsVisible = false;
		}
		else {
			MouseEvent.target.id = 'PinControl-HideButton';
			MouseEvent.target.setAttribute ( 'title' , _Translator.getText ( 'L.Marker.Pin.Control.onAdd.HideButton' ) );
			_Pins.show ( );
			_PinsVisible = true;
		}
		MouseEvent.stopPropagation ( );
	...
	*/
	...
	var _createButtonsDiv = function ( MainDiv, IsMinimized ) {
		...
		var ShowHideButton = L.DomUtil.create ( 'div', 'PinControl-Button', ButtonsDiv );
		ShowHideButton.setAttribute ( 'title' , _Translator.getText ( 'L.Marker.Pin.Control.onAdd.HideButton' ) );
		ShowHideButton.id = 'PinControl-HideButton';
		L.DomEvent.on ( ShowHideButton, 'click', _onClickShowHide );
